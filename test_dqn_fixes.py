...
def test_reward_system():
    ...
    for key, expected_value in expected_rewards.items():
        ...
        assert actual_value == expected_value, (
            f"Reward for {key} is {actual_value}, expected {expected_value}"
        )
...
def test_controller_integration():
    ...
    try:
        ...
        assert hasattr(controller, "current_state"), (
            "Controller should have current_state"
        )
...
def test_dqn_prediction():
    ...
    try:
        ...
        test_board = np.array([[0, 1, 0], [1, 0, 2], [2, 1, 0]])
        ...
        assert isinstance(prediction, int), (
            f"Prediction should be int, got {type(prediction)}"
        )
        ...
        assert padded_board.shape == (10, 10), (
            f"Padded board should be (10, 10), got {padded_board.shape}"
        )
...
def test_replay_buffer():
    ...
    try:
        ...
        assert len(mm.replay_buffer) == 1, (
            f"Replay buffer should have 1 item, got {len(mm.replay_buffer)}"
        )
        ...
        assert len(samples) == 1, (
            f"Should return all available samples, got {len(samples)}"
        )
